...
# 폰트 경로 설정 및 폰트 등록
font_path = font_path = 'fonts/나눔손글씨 희망누리.ttf'
...
class TodoScreen(Screen):
    ...
    def add_task(self, instance=None):
        ...
        task_box.add_widget(checkBox) #체크박스 추가
        task_box.add_widget(task_input) #입력란 추가
        task_box.add_widget(delete_button) #삭제 버튼 추가
...
class TimerScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.alarm_sound = SoundLoader.load("sounds/99DD78465CF6B4DE03.mp3")
        ...
        self.timer_button = Button(
            text=self.format_time(self.timer_seconds), # 현재 타이머 시간
            font_size=80,
            size_hint_y=0.3,
            color=(0.1, 0.6, 0.9, 1),
        ...
        self.start_button.bind(on_press=self.toggle_timer) #버튼 클릭 시 타이머에 시작 또는 정지
        button_layout.add_widget(self.start_button)

        # 10분 추가 버튼
        add_button = Button(text='+ 10분 추가', size_hint=(0.3, 1), font_size=30, font_name='NanumHandwriting')
        add_button.bind(on_press=self.add_ten_minutes) #버튼 클릭 시 타이머에 10분 추가
        button_layout.add_widget(add_button)

        # 뒤로 가기 버튼
        back_button = Button(text='뒤로', size_hint=(0.3, 1), font_size=30, font_name='NanumHandwriting')
        back_button.bind(on_press=self.go_back) #버튼 클릭 시 메뉴로 돌아가기
...
class StopwatchScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.timer_label = Label(text="00:00:00", font_size=72)
        ...
        # 스톱워치 시작 버튼
        self.start_button = Button(text='시작', size_hint=(0.3, 1), font_size=30, font_name='NanumHandwriting')
        self.start_button.bind(on_press=self.toggle_stopwatch)
        button_layout.add_widget(self.start_button)

        # 스톱워치 정지 버튼
        self.stop_button = Button(text="정지", size_hint=(0.2, 0.1), pos_hint={'center_x': 0.7, 'center_y': 0.2})
        self.stop_button.bind(on_press=self.toggle_stopwatch)  # 정지 버튼 클릭 시 스톱워치 시작/정지
    ...
    def stop_stopwatch(self, instance):
        #스톱워치 정지
        self.is_running = False  # 스톱워치 상태를 정지로 설정
        self.start_button.text = '시작'  # 버튼 텍스트 변경
